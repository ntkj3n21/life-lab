...
function Resolve-A2Binary {
    ...
    $found = Get-ChildItem (Join-Path $env:LOCALAPPDATA "Microsoft\WinGet") `
        -Recurse `
...
foreach ($url in $urls) {
    ...
    try {
        ...
        # yt-dlp can legitimately write warnings to stderr, so temporarily allow
        # the native process to finish and classify by exit code + captured stderr.
