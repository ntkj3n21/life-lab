...
import { NoteFilters, type NoteFilterValues } from "../components/NoteFilters";
...
type AppliedNoteFilters = NoteFilterValues;
...
export function NotesPage() {
  ...
  const createTaskResolutionInFlightRef = useRef(false);
  ...
  const [preparingTaskNoteId, setPreparingTaskNoteId] = useState<number | null>(
    null,
  );
  ...
  const [editingOrganizationNoteId, setEditingOrganizationNoteId] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    void getNotes({
      ...
      hasTimestamp: filters.hasTimestamp,
  ...
  function buildNoteQuery(
    ...
    return {
      ...
      categoryId: targetFilters.categoryId,
      tagIds:
        targetFilters.tagIds.length > 0 ? targetFilters.tagIds : undefined,
      hasTimestamp: targetFilters.hasTimestamp,
      sortBy: targetFilters.sortBy,
      sortDirection: targetFilters.sortDirection,
    };
  }
  ...
  async function handleSaveEdit(noteId: number) {
    ...
    try {
      await updateNoteRequest(noteId, {
        content,
      });
    ...
    try {
      await reloadNotesAfterMutation(page);
      setLoadErrorMessage(null);
    } catch (error) {
      setLoadErrorMessage(getErrorMessage(error));
    } finally {
  ...
  function handleStartOrganizationEdit(note: Note) {
    handleCancelEdit();
    setEditingOrganizationNoteId(note.id);
    setOrganizationErrorMessage(null);
  }
  ...
  async function handleSaveOrganization(
    ...
    try {
      const organization = await updateNoteOrganization(noteId, {
        categoryId,
        tagIds,
      });
  ...
  async function handleRequestDelete(note: Note) {
    if (isMutating) {
      return;
    }
    ...
    try {
      const impact = await getNoteDeleteImpact(note.id);
  ...
  async function handleConfirmDelete() {
    ...
    try {
      await deleteNoteRequest(pendingDelete.note.id);

      if (editingNoteId === pendingDelete.note.id) {
        handleCancelEdit();
      }

      if (editingOrganizationNoteId === pendingDelete.note.id) {
        setEditingOrganizationNoteId(null);
      }
  ...
  async function handleCreateTask(note: Note) {
    if (createTaskResolutionInFlightRef.current) {
      return;
    }

    createTaskResolutionInFlightRef.current = true;
    setPreparingTaskNoteId(note.id);
    setActionErrorMessage(null);

    try {
      const resolution = await openNoteContext(note.id);

      if (resolution.navigationMode !== "WORKSPACE") {
        return;
      }
      ...
      createTaskResolutionInFlightRef.current = false;
  ...
  function handleCategoryChange(change: CategoryManagerChange) {
    ...
    if (change.type === "renamed") {
      setNotes((current) =>
        current.map((note) =>
          note.category?.id === change.category.id
            ? {
                ...note,
    ...
    if (filters.categoryId === change.category.id) {
      applyFilterChange({
        ...filters,
  ...
  function handleTagChange(change: TagManagerChange) {
    ...
    setNotes((current) =>
      current.map((note) => ({
        ...note,
        tags: note.tags.filter((tag) => tag.id !== change.tag.id),
      })),
    );

    if (filters.tagIds.includes(change.tag.id)) {
      applyFilterChange({
        ...filters,
        tagIds: filters.tagIds.filter((tagId) => tagId !== change.tag.id),
      });
      return;
    }
            ...
            {notes.map((note) => (
              <div key={note.id}>
                <NoteCard
                  ...
                  editingContent={
                    editingNoteId === note.id ? editingContent : ""
                  }
                  ...
                  onCreateTask={(selectedNote) =>
                    void handleCreateTask(selectedNote)
                  }
                  ...
                  isCreatingTask={preparingTaskNoteId === note.id}
                  isCreateTaskDisabled={preparingTaskNoteId !== null}
                  onOpenDetail={(noteId) => navigate(`/notes/${noteId}`)}
                />

                {editingOrganizationNoteId === note.id && (
                  <NoteOrganizationEditor
                    ...
                    onSave={(categoryId, tagIds) =>
                      handleSaveOrganization(note.id, categoryId, tagIds)
                    }
                    onCancel={() => {
                      if (!isMutating) {
                        setEditingOrganizationNoteId(null);
